...
from remote import BASH_TOOL


OBJECTIVE = (
    "Use ordinary bash with environment=sandbox and target=default to read app/numbers.txt "
    "under /workspace/task. Compute its sum and write app/sum.txt containing only the integer and a newline. "
    "Use bash with environment=work (cwd /work) to create surface/report.md and surface/notes.md "
    "describing the observed result and commands actually performed. Read back all produced files to verify "
    "them and give a concise final answer. Use at most six model turns and tool timeout at most 30 seconds. "
    "Work and Sandbox have independent files and identities."
)
...
def main(args):
    ...
    source = worker.read_text().replace("INDEPENDENT NODE POLICY", "Work on the objective using ordinary Bash in the designated environment. Keep observed evidence in surface/report.md and surface/notes.md under /work. Task files are in app under /workspace/task in Sandbox; never invent evidence.")
    ...
    try:
        ...
        assert result["state"] == "handed_off", result
        saved = output / "saved-task-copy"
        fixture.call("restore-resource", work, "app", saved, "--target", "default")
        assert (saved / "sum.txt").read_text().strip() == "10"
        assert not (userspace / "sum.txt").exists()
        assert (userspace / "numbers.txt").read_text() == "2\n3\n5\n"
        for name in ("report.md", "notes.md"):
            content = (work / "surface" / name).read_bytes()
            assert content.strip()
            captured = subprocess.check_output(["git", "--git-dir=" + str(work / ".loom/versions.git"), "show", result["revision"] + ":surface/" + name])
        ...
        for effect in effects:
            ...
            if effect["kind"] == "tool.exec":
                environment = json.loads(effect["request"])["environment"]
                shell_domains.add(environment)
                receipt = settled["receipt"] if environment == "sandbox" else settled
                ...
                if environment == "sandbox":
                    ...
                    for artifact in receipt["artifacts"]:
                        ref = artifact["record_ref"]
                        raw = (work / ".loom/records" / ref["sha256"]).read_bytes()
                        assert hashlib.sha256(raw).hexdigest() == ref["sha256"]
        assert shell_domains == {"work", "sandbox"}
        ...
        observations.update(round=result, usage=usage, checks=["independent sum(2,3,5)=10", "distinct Work and Sandbox environments", "saved Target copy with unchanged shared source", "report/notes Git bytes", "native provider usage", "durable completed effects", "no pending", "remote operation containers absent"])
