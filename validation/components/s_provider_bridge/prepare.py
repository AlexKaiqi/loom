...
def main():
 ...
 auth=json.loads((ROOT/'validation/components/provider/evidence/host-transport-root-001/result.json').read_text());assert auth['status']=='PASS' and auth['source_unchanged']
 for file,digest in auth['input_sha256'].items():p=Path(file);cp(p,ws/p.relative_to(ROOT),digest)
 cp(ROOT/'lore_session/provider.py',ws/'lore_session/provider.py')
 ...
 extras.extend([ROOT/'design/g4/s-provider-bridge-author-001/contract.md',ROOT/'design/g4/s-provider-bridge-author-001/preparation.json',ROOT/'validation/components/provider/evidence/host-transport-root-001/result.json'])
 ...
 record={'scope':'WIRE_PI_MECHANISM_PREPARATION_ONLY','inputs':rows,'links':links,'generated':generated};save(out/'before.json',record);argv=[sys.executable,'-B',str(ws/'validation/components/s_provider_bridge/bridge_probe.py'),str(out/'actual')];record['argv']=argv
