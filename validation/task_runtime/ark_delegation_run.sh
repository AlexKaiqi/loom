...
$PY -m lore_task admit --root "$ROOT" --task-id parent-1 --foreign-id obj-001 --kind task.objective.set \
...
$PY -m lore_task relay --from-root "$ROOT" --from-task parent-1 --to-root "$ROOT" --to-task child-1 \
  --foreign-id d-001 --kind task.delegated --payload "$DELEGATED" > "$EV/07-relay-delegation.json"
...
$PY -m lore_task relay --from-root "$ROOT" --from-task child-1 --to-root "$ROOT" --to-task parent-1 \
  --foreign-id d-002 --kind task.reported --payload "$REPORTED" > "$EV/09-relay-report.json"
