...
"""Independent checker for a coding task directory.
...
Usage: python3.12 verify_coding_task.py <task-dir>
"""
import argparse
import hashlib
import json
...
def main() -> int:
    ...
    check("workspace_declared", len(task.get("workspaces", [])) == 1, json.dumps(task.get("workspaces")))
    workspace = Path(task["workspaces"][0]["path"])
    ...
    check("tool_targets_workspace", tools and all(t.get("target") == "workspace" for t in tools),
          json.dumps([t.get("target") for t in tools]))
    check("cwd_is_workspace", all(t.get("cwd") == str(workspace) for t in tools),
    ...
    check("add_py_present", (workspace / "add.py").is_file())
    check("test_py_present", (workspace / "test_add.py").is_file())
    ...
    proc = subprocess.run([sys.executable, "test_add.py"], cwd=str(workspace),
    ...
    print(json.dumps({"task": str(base), "workspace": str(workspace), "kinds": kinds, "ok": ok}, ensure_ascii=False))
