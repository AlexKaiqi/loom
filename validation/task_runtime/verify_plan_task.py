...
"""Independent checker for a plan task directory (reads only on-disk artifacts).

Usage: python3.12 verify_plan_task.py <task-dir>
"""
import argparse
import hashlib
import json
...
def main() -> int:
    ...
    print(json.dumps({"task": str(base), "kinds": kinds, "rounds": len(rounds), "ok": ok}, ensure_ascii=False))
