...
"""Offline case: coding mode routes execution to an authorized workspace range.

Covers: unauthorized target refusal, real edit + test in the workspace, declared
...
STEPS = [
    json.dumps({"action": {"type": "shell", "target": "workspace:secret",
                           "script": "echo should-not-run > /tmp/lore-escape.txt"}}),
    json.dumps({"action": {"type": "shell", "target": "workspace",
    ...
    json.dumps({"action": {"type": "emit", "kind": "task.completed", "payload": {"evidence_refs": ["add.py"]}}}),
]
...
def main() -> int:
    ...
    check("unauthorized_target_refused", tools[0]["exit"] == 126 and "unauthorized" in tools[0]["stderr"],
          json.dumps(tools[0]))
    check("refusal_records_target", tools[0]["target"] == "workspace:secret", json.dumps(tools[0].get("target")))
    check("no_escape_effect", not Path("/tmp/lore-escape.txt").exists())
    check("workspace_used", tools[1]["target"] == "workspace" and tools[1]["cwd"] == str(workspace),
          json.dumps(tools[1]))
    check("code_written", (workspace / "add.py").is_file() and (workspace / "test_add.py").is_file())
    ...
    print(json.dumps({"task_root": str(root), "workspace": str(workspace), "kinds": kinds}, ensure_ascii=False))
