...
"""Independent checker for a delegation pair (parent + child work directories).

Usage: python3.12 verify_delegation_works.py <root> <parent-work-id> <child-work-id>
"""
import argparse
import json
...
def main() -> int:
    ...
    check("parent_delegated_once", pk.count("work.delegated") == 1, str(pk))
    check("parent_received_report", pk.count("work.reported") == 1, str(pk))
    check("parent_completed_once", pk.count("work.completed") == 1, str(pk))
    check("parent_order", pk.index("work.delegated") < pk.index("work.reported") < pk.index("work.completed"), str(pk))
    check("parent_content_untouched", not any((parent / "surface" / "content").iterdir()))

    check("child_got_delegation", ck.count("work.delegated") == 1, str(ck))
    check("child_reported_once", ck.count("work.reported") == 1, str(ck))
    check("child_completed_once", ck.count("work.completed") == 1, str(ck))
    ...
    parent_relations = json.loads((parent / "work.json").read_text()).get("relations", {})
    child_relations = json.loads((child / "work.json").read_text()).get("relations", {})
    check("parent_grants_child", any(g.get("peer") == args.child and "work.delegated" in g.get("kinds", [])
                                     for g in parent_relations.get("grants", [])), json.dumps(parent_relations))
    check("parent_wants_child_report", any(w.get("peer") == args.child and "work.reported" in w.get("kinds", [])
                                           for w in parent_relations.get("wants", [])), json.dumps(parent_relations))
    check("child_wants_parent_delegation", any(w.get("peer") == args.parent and "work.delegated" in w.get("kinds", [])
