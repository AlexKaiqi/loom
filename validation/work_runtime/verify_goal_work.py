...
"""Independent checker for a goal work directory: reads only on-disk artifacts.

Usage: python3.12 verify_goal_work.py <work-dir> [--expect-sum 10]
...
def main() -> int:
    ...
    check("layout_version", work.get("layout_version") == 1, json.dumps(work.get("layout_version")))
    check("harness_digest_matches", tree_digest(base / "harness") == work["harness"]["digest"],
          "%s != %s" % (tree_digest(base / "harness"), work["harness"]["digest"]))
    ...
    check("single_completion", kinds.count("work.completed") == 1, str(kinds))
    check("declared_kinds_only",
          set(kinds) <= {"sys.tool.result", "work.objective.set", "user.message", "work.phase.changed", "work.completed"},
          str(set(kinds)))
    ...
    print(json.dumps({"work": str(base), "facts": len(facts), "kinds": kinds, "ok": ok}, ensure_ascii=False))
