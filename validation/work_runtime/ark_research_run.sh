...
EV=${EV_DIR:-validation/work_runtime/evidence/ark-research-001}
ROOT="$EV/root"
rm -rf "$EV"; mkdir -p "$EV"

$PY -m lore_work create --root "$ROOT" --work-id research-1 --harness lore_harness/research > "$EV/01-create.json"
...
$PY -m lore_work admit --root "$ROOT" --work-id research-1 --foreign-id q-001 --kind research.question.set \
...
for i in 1 2 3 4; do
  ...
  $PY -m lore_work run --root "$ROOT" --work-id research-1 --provider ark --max-tokens 10000 --max-steps 4 --timeout 280 > "$EV/03-run-r$i.json" || true
...
for line in open(sys.argv[1]+"/surface/facts.jsonl"):
    f=json.loads(line)
    if f["kind"]=="work.completed": done=True
