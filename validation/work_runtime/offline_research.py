...
def main() -> int:
    ...
    check("completed_derived", kinds.count("work.completed") == 1 and kinds[-1] == "work.completed", str(kinds))
    ...
    check("completion_blocked_by_invalid", "work.completed" not in kinds2, str(kinds2))
    round_mod.run_round(base2, provider.FauxProvider([
        json.dumps({"action": {"type": "emit", "kind": "research.source.added",
    ...
    check("self_heals_after_invalid_source", kinds2b.count("work.completed") == 1, str(kinds2b))
    ...
    check("derived_completion", kinds3.count("work.completed") == 1 and kinds3[-1] == "work.completed", str(kinds3))
