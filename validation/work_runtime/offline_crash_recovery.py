...
COMPLETE = [
    ...
    json.dumps({"action": {"type": "emit", "kind": "work.completed", "payload": {"evidence_refs": ["final.txt"]}}}),
]
...
def main() -> int:
    ...
    check("completed_after_recovery", kinds.count("work.completed") == 1, str(kinds))
    ...
    check("double_recover_keeps_admitted_facts",
          len(after_first) == len(after_second) == 1 and after_second[0]["kind"] == "work.objective.set",
