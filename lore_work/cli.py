...
written into the work directory or into evidence.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="lore_work", description="Work-directory runtime")
    sub = parser.add_subparsers(dest="command", required=True)

    create = sub.add_parser("create", help="create a work directory from a harness template")
    create.add_argument("--root", required=True)
    create.add_argument("--work-id", required=True)
    create.add_argument("--harness", required=True)
    create.add_argument("--userspace", action="append", default=[],
                        help="authorized file range outside the work dir (repeatable; path)")

    admit = sub.add_parser("admit", help="admit an external fact through Fact Admission")
    admit.add_argument("--root", required=True)
    admit.add_argument("--work-id", required=True)
    ...
    status = sub.add_parser("status", help="print work status")
    status.add_argument("--root", required=True)
    status.add_argument("--work-id", required=True)

    facts = sub.add_parser("facts", help="print the fact stream")
    facts.add_argument("--root", required=True)
    facts.add_argument("--work-id", required=True)
    ...
    relate = sub.add_parser("relate", help="declare cross-work wants/grants (PEER:KIND[,KIND])")
    relate.add_argument("--root", required=True)
    relate.add_argument("--work-id", required=True)
    relate.add_argument("--want", action="append", default=[])
    relate.add_argument("--grant", action="append", default=[])

    register = sub.add_parser("register", help="(re-)register an existing work in this root's host authority (import/adopt)")
    register.add_argument("--root", required=True)
    register.add_argument("--work-id", required=True)

    relay = sub.add_parser("relay", help="relay one event through another work's Fact Admission (authorized by wants+grants)")
    relay.add_argument("--from-root", required=True)
    relay.add_argument("--from-work", required=True)
    relay.add_argument("--to-root", required=True)
    relay.add_argument("--to-work", required=True)
...
def main(argv=None) -> int:
    ...
    if args.command == "create":
        userspaces = []
        for raw in args.userspace:
            path = os.path.abspath(os.path.expanduser(raw))
            if not os.path.isdir(path):
                print(json.dumps({"error": f"userspace is not a directory: {path}"}), file=sys.stderr)
                return 2
            userspaces.append({"id": os.path.basename(path), "path": path, "mode": "rw"})
        base = layout.create_work(args.root, args.work_id, args.harness, userspaces=userspaces)
        print(json.dumps({"created": str(base), "work": layout.read_json(layout.paths(base)["work"])},
                         ensure_ascii=False, indent=2))
        return 0

    if args.command == "relay":
        result = round_mod.relay(
            layout.work_path(args.from_root, args.from_work),
            layout.work_path(args.to_root, args.to_work),
            args.foreign_id, args.kind, _json_arg(args.payload))
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0

    base = layout.work_path(args.root, args.work_id)
    ...
    if args.command == "register":
        entry = layout.register_authority(args.root, args.work_id, base)
    ...
    if args.command == "facts":
        for fact in __import__("lore_work.facts", fromlist=["read_facts"]).read_facts(base):
