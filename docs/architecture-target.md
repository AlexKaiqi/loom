# 目标架构：Work、上下文与共享执行环境

状态：已确认的重构设计，尚未完整实现。产品目标和不变量以 [SPEC.md](../SPEC.md) 为准；[architecture.md](architecture.md)、源码、锁文件和 [contracts/](contracts/) 描述当前实现。本文给出组件边界、目录、协议、失败语义和待验收事项，不把设计当作完成证据。
...
| 事件与状态存储 | 完整事实、输入身份、待处理责任、执行状态、订阅游标、内容版本关联 | SQLite 事务及独立存储模块；无需强制拆成网络服务 |
...
查看历史保留后续记录。从旧点继续推进需要新的恢复或分叉关系。文件回退不撤销外部世界，未知模型或工具效果保持原身份和责任，不能因进程消失、租约超时或回拨而自动重发。精确恢复中间工具阶段需要模型组件原生支持，不能由 Runtime 再造一套循环猜测。
