...
| VO04 | **凭据不落库**：任务目录、证据、日志、错误消息中无密钥值；仅出现变量名 | 错误消息回显 Header；证据带 key | 扫描 + 脱敏错误注入 | 对齐 glossary:100-102 |
| VO05 | **离线可复现**：无网络时 `fake` 适配器驱动整条任务面链路，结果确定性可复现 | 离线即不可运行、依赖真实端点 | 断网复跑 ×2 字节比较 | 与 VO01 共用套件 |
...
| VO16 | **会话设施零驻留**：会话结束后任务专属媒体进程/连接释放；共享服务可常驻 | 每任务常驻 Agent | 进程/连接观测 | 对齐 v5:90 |
| VO17 | **会话层不写任务状态**：会话层被拒写 `harness/`、`surface/head`、`ledger/`；只能经 event 入口 | 会话层直接改文件当状态 | 挂载 + 写尝试 + 字节不变 | 对齐 landing §8 |
...
| VO30 | **零驻留**：两次触发之间无任务专属常驻进程/长连接；调度工具是共享设施 | 轮内 `sleep` 常驻；每任务一个调度器进程 | 进程/连接观测 | 对齐 v5:90；与 VO16 独立（那测会话，这测定时） |
| VO31 | **权威在数据、crontab 是派生物**：日程存任务目录；换宿主后 runtime 重新 materialize 并继续生效；旧宿主条目无需迁移 | 日程只存在宿主 crontab 里；换宿主丢失 | 迁移前后对比 + 宿主 crontab 检查 | 与对话记录同一模式（外部机制可替换） |
...
| VO36 | **cron 不写事实**：调度条目只调用 runtime 入口；任务/模型无法直接改宿主 crontab；事实流写者唯一 | 条目直接 append facts；任务写 crontab | 宿主 crontab 内容 + 事实流写者观测 | 安全/单写者必须项 |
...
| VO38 | **到点不自动杀**：到达默认/软预算时**不杀**，harness 可继续；只有 harness 判定或资源上限才终止（终止时进程组全消失） | 30 s 无条件杀，误杀长任务；只杀直接子进程 | 事实流 + 进程组观测 | 复用 M03 进程组先例 |
