...
> **v2 补全**：[task-directory-landing.md](task-directory-landing.md) 补本文件没有回答的另一半——
...
目录结构是概念边界在磁盘上的投影：框架区对所有任务同构，runtime 语义只有一套；
任务的个性（目录结构、投影方式、推进方式）只落在 `surface/content/` 内部。

## 目录树

```
<tasks-root>/<task-id>/
├── task.json                  # 身份与绑定：授权身份、model refs、沙箱绑定、
...
**T2 不进任务目录**：沙箱实例、轮执行现场放 runtime 临时区，按 Round 分配释放。
目录里永远只有"接续所需的持久物"——**拷走目录 = 搬走完整任务**（可移植性不变量）。
...
| `session/`、`ledger/`、`task.json` | Runtime / 管理面 | T0 |
| `derived/` | Runtime 重建 | T1（可删） |
| （目录外）沙箱实例 | Runtime 按 Round 分配释放 | T2 |

## 设计陈述

1. **不同任务的"不同目录结构"只落在一个点**：`surface/content/` 内部（助手任务长成
   `journal/ + index.md`，编码任务长成 `src/ + tests/`，由各自 conventions 决定）。
...
| `task.json` 字段 | model refs、沙箱绑定、受理关系、授权身份 |

Runtime 侧拓展点（工具执行、事件底座）不在任务目录——它们是机制，不是任务个性。
...
| `harness/` | ✅ | 策略代码演进需人审；`task.json` 的 digest 钉住准入版本，git 管"怎么变来的" |
| `task.json` | ✅ | 绑定/授权/关系变更史 = 管理审计 |
| `surface/facts.jsonl` | ❌ | 已是 append-only 历史，git 版本化冗余；重放靠它自己 |
| `session/`、`ledger/` | ❌ | 观测域/运行记录，无 diff 价值 |
| `derived/` | ❌ | 可重建，版本化无意义 |

整个任务目录 = 一个 git repo（`facts/` `session/` `ledger/` `derived/` 进 ignore）——
repo 边界 = 任务边界，可移植性不破坏。
...
- **`task.json`**：唯一"既是数据又是配置"的文件，git 管变更史。

## 待定机制题（进入组件合同时逐项裁决）

1. 事实流分片策略（按量/按时间滚动），接续重放语义不变；
2. revision 粒度（默认每 Round 一个快照；工具调用级是否留痕交 conventions）；
3. 关系允许的双向性：B 受理 A 的事实，允许记录记在谁的 task.json
   （倾向 B 声明 wants、A 侧 grants，归 admission 合同）；
4. 归档区位置：框架不设 `surface/archive/`，由各任务 conventions 决定
   （助手任务 `content/archive/`，编码任务可不需要）；
