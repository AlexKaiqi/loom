...
- N1 长程任务**在等待中活、在执行时算**（归零）→ 活的单位 ≠ 执行的单位 → Task / Round
- N2 一切因果**可恢复、可审计、可协作** → 事实有属地、准入有唯一门、语义有身份
  → Surface / Fact Admission / Fact Contract / Event
- N3 推进策略**因任务而异、可对比** → 策略与机制分离 → Harness / Runtime
- N4 模型是**瞬时能力** → 意图与供给分离 → Model Ref / Model Routing；供给的节拍 → Step
- N5 执行**开放长** → 权能必须短命 → Round Grant；执行要有承载 → Task Environment / Workspace
- N6 模型**只能看见被供给的内容** → 内容/形式分离 + 按需深挖
  → Projection / Presentation / Context Organization
- N7 交互史**是观测不是因果**；数据**可重建性不同** → Session；T0/T1/T2
- N8 轮要能**开始和结束**、任务要能**"不活"** → Trigger Condition / Attempt / Archive
- N9 跨任务协作**无中心行动者** → 各任务 Harness 声明受理（N3+N2 推论）

## 目标形态

一个 Task 是推进所需的全部持久物，"活"的唯一单位：在册即活，Runtime 关注它——
求值其 Surface 上的 Trigger Condition、执行其 Fact Admission 规则。任务携带自己的 Harness（推进定义）：
面形态约定、Projection 策略、Round 逻辑、Fact Admission 规则、Trigger Condition 与前置逻辑代码——
不同任务有不同的目录结构、投影方式与推进方式，这些都属于任务目录。
推进只由 Event 驱动：外来事件（用户、时钟、外部系统、其他任务的 Surface）经 Fact Admission
落为面的事实；Trigger Condition 满足时任务进入一个 Round——Harness 逻辑前置（可选）将事实
转换为面内容，若干 Step 完成模型工作（模型按 Model Ref 经 Model Routing 供给），新事实
落面，Round 结束资源归零。一切事实落面；Round 之间任务休眠——纯数据驻留，Trigger Condition
仍被求值；接续 = 从 T0 重放（含 Harness）。多任务协作 = 面间事件受理，按各自
Harness 声明，无中心行动者。
...
- **Runtime（运行时）**。概念：机制的所有者——"怎么执行"归它。
  定义：事件底座、任务登记与关注、任务环境生命周期、解释 Harness 并执行推进、
  记录与恢复。边界：不内置任何策略；策略全部来自任务携带的 Harness。
- **Harness（推进定义）**。概念：策略的所有者——"看什么、何时推进、如何接线"归它；
  回答"任务的个性放在哪"。定义：面形态约定（目录结构）+ Projection 策略 +
  Round 逻辑（含结束条件）+ Fact Admission 规则与 Trigger Condition + 前置逻辑代码；内容寻址、
  随任务登记固定，改一字节即新版本；可从模板实例化，登记后是任务自己的数据。
  边界：与 Runtime 的分界 = 策略 vs 机制；模型默认不可自改自己的 Harness。

### 活与推进
- **Task（任务）**。概念：回答三问——什么是"同一个任务"（身份）、什么算"活着"
  ...
  Task 与 Surface 1:1。边界：模型与 Runtime 不在任务内（轮执行期间供给的瞬时能力）；
...
- **Round（轮）**。概念：授权、资源、预算、恢复需要共同的边界——有界性是四者的
  ...
  边界：轮有结束条件，任务没有；attempt 是轮内重试，不改轮身份。
...
- **Fact Admission（受理）**。概念：外来事实与一个面之间的唯一门——门上只挂三件事：
  ...
  事实"。定义：外来事件经它成为面流事实；规则由任务自己的 Harness 声明；跨任务
  受理需对方允许（授权语义，机制待定）；受理不改变事实内容——语义身份由 Fact Contract
  决定，它只决定收不收。边界：**门外是世界，门内是面的事实流；门后的一切
  （Trigger Condition 求值、Round 推进）都不属于它。**

### 行动与承载
- **Tool（工具）**。概念：模型行动的两类通道——任务域执行与 Runtime 域操作；
  跨界必须显式且限时。定义：普通工具（bash / MCP 同类，任务域执行）∥
  受控设施（emit/publish 类 Runtime 薄包装，需 Round Grant）。
- **Task Environment（任务环境）**。概念：执行的承载——可分配、可归零，
  与任务数据的持久性分离。定义：挂载 Workspace 的执行环境；沙箱实例按 Round
  分配、轮间归零、可短暂保温。
- **Workspace**。概念：授权的文件范围——"能碰什么"与"在哪里执行"分离。
  定义：文件范围引用（owner/kind/身份/版本）。
- **Round Grant（轮授权）**。概念：最小授权——模型执行开放长，权能必须比它短命。
  定义：Round 作用域的权能（受控设施、资源）；Round 结束失效。

### 模型供给
- **Model Ref（模型引用）**。概念：意图与供给分离的需求侧——换模型 = 换引用，
  多策略对比免费。定义：任务按用途声明的模型引用（reasoning / recall / embedding 等）；
  秘密与端点不在任务内。
...
Task ≠ Surface（数据全体 vs 其内容+事实平面）｜Task ≠ Round（生命周期全体 vs 有界单位）｜
...
（任务域 vs Runtime 域）｜Model Routing ≠ Model Ref（供给侧 vs 需求侧意图）｜
任务的"活" = 在册+被关注 ≠ 进程存活｜**agent ≠ Task**（agent 是任务在 Round 执行
期间的临时形态——模型在环；任务才是持久单位。系统无常驻 agent；模型不在任务数据内）

## 墓碑（已废词，勿复活）

- **orchestration**：并入 **Harness**——v5:46 本义即"Harness 推进定义中涉及多个
  Surface 的普通代码"；跨任务接线 = Harness 的 Fact Admission 规则与触发条件，
  不再是独立登记实体。
- **run**：概念正确（自包含、在册即活）但词带过程味；概念并入 Task。
- **turn**：生命周期义归 Task，有界义归 Round。废因：消息族"会话轮次"歧义 + 参照实现授权机制包袱。
- **reaction**：并入 Round 的 Harness 逻辑前置阶段。
- **delivery / input / advance**：统一为"Event + Fact Admission"。
- **series**：被 Task 取代（即原"跨轮持久身份"）。
- **execution（名词）**：降为动词。
