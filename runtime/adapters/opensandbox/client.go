...
func (c *Client) checkDestination(expected Binding) error {
	actual := c.Binding()
	if expected.ServiceID == "" || expected != actual {
...
func (c *Client) execute(ctx context.Context, req Request, directory, artifacts string, identity os.FileInfo, inputDigest string, result *Result, checkpoint func(Checkpoint) error) error {
	...
	var err error
	if req.SandboxID == "" {
		allocation, e := c.Acquire(ctx, req.OperationID, req.Target)
		if e != nil {
			return e
		}
		result.SandboxID = allocation.ID
	} else {
		allocation, e := c.InspectAllocation(ctx, req.AllocationRequestID, req.SandboxID)
		if e != nil {
			return e
		}
		if allocation.ID != req.SandboxID || allocation.State == "absent" {
			return errors.New("original allocation missing")
		}
		result.SandboxID = allocation.ID
	}
	record := func(phase string) error {
		...
		return checkpoint(Checkpoint{Phase: phase, Binding: result.Binding, SandboxID: result.SandboxID, ExecutionID: result.ExecutionID, NativeResult: func() map[string]any {
			if phase == "native_result" {
				return map[string]any{"exit_code": result.ExitCode, "native": result.Native, "output_stream": "combined"}
			}
			return nil
		}(), Artifacts: result.Artifacts, Readiness: result.Readiness})
	}
	if err = record("created"); err != nil {
		return err
	}
	sb, err := upstream.ConnectSandbox(ctx, c.connection(), result.SandboxID, upstream.ReadyOptions{Timeout: c.config.RequestTimeout})
	if err != nil {
		return err
	}
	defer sb.Close()
	...
	remote := "/workspace/task"
	if err = command(ctx, sb, "rm -rf "+remote+" && mkdir -p "+remote+" && tar -xf /tmp/loom-input.tar -C "+remote+" && chown -R 65534:65534 "+remote+" && rm /tmp/loom-input.tar"); err != nil {
		return err
	}
	uid := uint32(65534)
	share := true
	binds := []upstream.BindMount{}
	for _, alias := range req.ReadOnlyPaths {
		if !regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9._-]*$`).MatchString(alias) {
			return errors.New("invalid readonly resource alias")
		}
		binds = append(binds, upstream.BindMount{Source: remote + "/" + alias, Dest: remote + "/" + alias, ReadOnly: true})
	}
	session, err := sb.IsolationCreate(ctx, upstream.CreateIsolatedSessionRequest{Workspace: upstream.IsolatedWorkspaceSpec{Path: remote, Mode: "rw"}, Profile: "strict", Binds: binds, ShareNet: &share, Uid: &uid, Gid: &uid, EnvPassthrough: &upstream.EnvPassthroughSpec{Mode: "allow", Keys: []string{}}, IdleTimeoutSeconds: ttl})
	if err != nil {
		return err
	}
	// Save the native session identity before any probe or task invocation. A
	// failed probe must remain cancellable at its original destination.
	result.ExecutionID = session.SessionID() + "/"
	if err = record("session_created"); err != nil {
		return err
	}
	readOnly, err := json.Marshal(append([]string{}, req.ReadOnlyPaths...))
	if err != nil {
		return err
	}
	preflight, err := session.Run(ctx, upstream.IsolatedRunRequest{Code: "cd " + remote + " && /usr/local/bin/python -I -c " + quote(codeProbe) + " " + quote(string(readOnly)), TimeoutSeconds: 30}, nil)
	if err != nil {
		return err
	}
	result.Native["preflight"] = preflight
	probe, err := json.Marshal(preflight)
	if err != nil {
		return err
	}
	probeArtifact, err := save(filepath.Join(artifacts, "readiness.json"), strings.NewReader(string(probe)))
	if err != nil {
		return err
	}
	probeArtifact.MIMEType = "application/json"
	result.Artifacts = append(result.Artifacts, probeArtifact)
	...
	if result.Readiness, err = parseReadiness(preflight.Text(), req.ReadOnlyPaths); err != nil {
		return err
	}
	if err = record("prepared"); err != nil {
		return err
	}
	run, err := session.RunBackground(ctx, "cd "+remote+" && env -i PATH=/usr/local/bin:/usr/bin:/bin HOME=/tmp LANG=C.UTF-8 /bin/bash --noprofile --norc -c "+quote(req.Script))
	if err != nil {
		return err
	}
	...
	// Native completion is durable before output capture and allocation cleanup.
	if err = record("native_result"); err != nil {
		return err
	}
	if err = session.Delete(ctx); err != nil {
		return err
	}
	if err = record("execution_fenced"); err != nil {
		return err
	}
	...
	// Resource cleanup is independent of the already saved native result/content.
	if req.SandboxID != "" {
		return nil
	}
	if releaseErr := c.Release(ctx, result.SandboxID); releaseErr != nil {
		result.Error = c.redact(releaseErr)
	} else {
