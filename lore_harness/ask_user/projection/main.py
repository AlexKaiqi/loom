...
ACTION_PROTOCOL = base.action_protocol([
    ...
    '{"action":{"type":"emit","kind":"work.completed","payload":{"evidence_refs":["<file>"]}}}',
    base.FINAL_EXAMPLE,
])


def _state(facts):
    folded = base.fold(facts, latest=("work.objective.set",), collect=("ask.requested", "ask.answered"),
    ...
    return (objective_payload.get("objective"), objective_payload.get("acceptance", []),
            asked, answered, open_ask, folded["work.completed"])
...
def build(*, work, facts, new, head, content_dir, step=0, max_steps=1, rejected_final=None, userspaces=None, revision=None):
    ...
    lines += [
        ...
        "ask.requested exactly once and end the Round — the work then waits as data.",
        "2. When the open question has an answer above, use that answer and finish the work.",
        "3. Once acceptance is satisfied, emit work.completed with evidence_refs.",
    ...
    return {
        "system": (
            "You are the model inside one Round of an ask-user work. Ask instead of guessing; the answer "
