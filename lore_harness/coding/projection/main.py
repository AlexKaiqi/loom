...
Content directory = notes and reports (Surface). Userspace = the code under test
...
ACTION_PROTOCOL = base.action_protocol([
    '{"action":{"type":"shell","target":"userspace","script":"<sh script>"}}   run in the code userspace',
    ...
    '{"action":{"type":"emit","kind":"work.completed","payload":{"evidence_refs":["report.md"]}}}',
    base.FINAL_EXAMPLE,
])


def build(*, work, facts, new, head, content_dir, step=0, max_steps=1, rejected_final=None, userspaces=None, revision=None):
    folded = base.fold(facts, latest=("work.objective.set",),
    ...
    workspace_lines = []
    for userspace in (userspaces or []):
        root = userspace.get("path")
        listing = []
        try:
            listing = sorted(str(p.relative_to(root)) for p in Path(root).rglob("*") if p.is_file())[:30]
        except OSError:
            listing = ["(unreadable)"]
        workspace_lines.append("%s -> %s  files: %s" % (userspace.get("id"), root, ", ".join(listing) or "(empty)"))
    ...
    ] + ["- userspace: %s" % line for line in workspace_lines] + [
        "",
    ...
    lines += [
        "",
        "# Rules",
        "1. Code lives in the userspace; notes/reports live in content. Use `target` to choose.",
        ...
        "4. Only emit work.completed after a real passing test and with evidence_refs.",
    ...
    return {
        "system": (
            "You are the model inside one Round of a coding work. Edit code in the authorized userspace, run "
